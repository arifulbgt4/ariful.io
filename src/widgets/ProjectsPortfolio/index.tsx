...
import { PROJECTS, type Project } from "src/global/config";
...
const TechIcon = ({ name }: { name: string }) => {
  ...
  // Canvas / UI Experiments (generic design icon)
  if (
  ...
  // Default code icon for any other tech
  return (
    <svg
      className={iconClass}
      viewBox="0 0 24 24"
...
const ProjectsPortfolio = ({
  ...
  return (
    <section
      id="projects"
      className="relative z-[2] mt-10 space-y-6 rounded-xl border bg-card/80 p-4 shadow-sm backdrop-blur-md md:mt-12 md:p-6"
    >
      <header className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            {featuredOnly
              ? "A selection of my best work and notable contributions."
              : "A comprehensive overview of all products and experiments I have designed and developed."}
          </p>
        </div>
        {featuredOnly && (
          <div className="flex gap-2 text-sm">
            <Link
              href="/projects"
              className="inline-flex items-center rounded-md border px-3 py-1.5 text-xs font-medium hover:bg-accent hover:text-accent-foreground"
            >
              View all projects
            </Link>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {projects.map((project: Project) => (
          <article
            key={project.title}
            className="group flex flex-col justify-between rounded-lg border bg-background/80 p-5 transition-all duration-200 hover:-translate-y-1 hover:border-primary/60 hover:shadow-lg"
          >
            <div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {project.techStack.map((tech: string) => (
                <span
                  key={tech}
                  className="inline-flex items-center gap-1.5 rounded-md bg-muted px-2.5 py-1.5 text-[11px] font-medium text-foreground/80 transition-colors hover:bg-primary/10"
                >
                  <TechIcon name={tech} />
                  {tech}
                </span>
              ...
              <div className="mt-4 flex gap-3 border-t pt-4">
                {project.href && (
                  <Link
                    ...
                    className="inline-flex items-center justify-center rounded-md p-2 text-muted-foreground hover:bg-accent hover:text-primary transition-colors"
                    title="View Live"
                  >
                    <svg
                      className="h-4 w-4"
